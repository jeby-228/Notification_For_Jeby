...
type MemberDeviceToken struct {
	MemberID    uuid.UUID  `gorm:"type:uuid;index;foreignKey;constraint:OnUpdate:CASCADE,OnDelete:CASCADE" json:"member_id"` // 會員ID
	DeviceToken string     `gorm:"size:255;uniqueIndex:idx_member_device_token,composite:member_id" json:"device_token"`      // 設備Token
